-- ===================================================================
-- SCRIB3-OS Full Schema Migration
-- Plan v4 — Complete Supabase schema
-- Run AFTER existing migrations (003_support_requests.sql)
-- ===================================================================

-- ===================================================================
-- TEAM PROFILES (extends existing profiles table)
-- ===================================================================

-- Add OS-specific columns to existing profiles table if not present
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS pd_goals (
  ...
  title text NOT NULL,
  type text CHECK (type IN ('SMART', 'Course', 'Stretch')),
  target_date date,
...
CREATE TABLE IF NOT EXISTS pd_proof_of_excellence (
  ...
  client_id uuid,
  type text CHECK (type IN ('Client Recognition', 'Internal Win', 'Skill Milestone', 'Peer Recognition')),
  source_link text,
...
CREATE TABLE IF NOT EXISTS pd_instant_feedback (
  ...
  team_member_id uuid REFERENCES profiles(id) ON DELETE CASCADE,
  date date NOT NULL,
  type text CHECK (type IN ('praise', 'development')),
...
CREATE TABLE IF NOT EXISTS pd_coaching_notes (
  ...
  call_recording_id uuid REFERENCES pd_call_recordings(id),
  visibility text DEFAULT 'manager_only' CHECK (visibility IN ('manager_only', 'team_member', 'csuite')),
  created_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS client_profiles (
  ...
  account_health text CHECK (account_health IN ('🔴', '🟠', '🟡', '🟢', '⭐')),
  contract_type text CHECK (contract_type IN ('Monthly Remit', 'One Time', 'As-Needed')),
...
CREATE TABLE IF NOT EXISTS projects (
  ...
  status text DEFAULT 'Not Started' CHECK (status IN ('Active', 'In Progress', 'On Hold', 'Completed', 'Invoiced', 'Inactive', 'Not Started')),
  type text CHECK (type IN ('Monthly Remit', 'One Time', 'As-Needed')),
  start_date date,
  completion_date date,
  production_lead uuid REFERENCES profiles(id),
  creative_lead uuid REFERENCES profiles(id),
  account_lead uuid REFERENCES profiles(id),
  team_members uuid[] DEFAULT '{}',
  freelancers uuid[] DEFAULT '{}',
  unit text,
  sow_url text,
  linear_board_url text,
  pre_alignment_complete boolean DEFAULT false,
  pre_alignment_data jsonb,
  notes text,
  created_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS vendor_profiles (
  ...
  tax_form_type text CHECK (tax_form_type IN ('w9', 'w8ben-e')),
...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  status text DEFAULT 'submitted' CHECK (status IN ('submitted', 'validated', 'processing', 'paid', 'rejected')),
...
CREATE TABLE IF NOT EXISTS engagement_health (
  ...
  total_cost numeric GENERATED ALWAYS AS (team_costs + vendor_costs + expense_costs) STORED,
  profit numeric,
  margin_pct numeric,
  health_status text CHECK (health_status IN ('loss', 'break-even', 'acceptable', 'healthy', 'great')),
...
CREATE TABLE IF NOT EXISTS bandwidth_estimates (
  ...
  hourly_rate numeric,
  labour_cost numeric GENERATED ALWAYS AS (estimated_hours * hourly_rate) STORED,
  submitted_at timestamptz DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_xp_events_member ON xp_events(team_member_id);

-- ===================================================================
-- XP TRIGGER (auto-update profiles.xp on xp_events insert)
-- ===================================================================
...
BEGIN
  UPDATE profiles
  SET xp = (SELECT COALESCE(SUM(xp_awarded), 0) FROM xp_events WHERE team_member_id = NEW.team_member_id)
